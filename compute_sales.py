...
def compute_sale_cost(sale: Dict, price_catalogue: Dict) -> Tuple[float, List[str]]:
    ...
    for item in sale.get('items', []):
        ...
        if not product_name or not quantity:
            errors.append(
                f"Sale {sale.get('id', 'Unknown')} invalid: missing product or quantity"
            )
            continue
        ...
        try:
            ...
            errors.append(
                f"Invalid price for product '{product_name}' in price catalogue"
            )

    return total_cost, errors


def write_results(total_cost: float, execution_time: float, errors: List[str]) -> None:
